...
const PERMISSIONS = [
  { name: 'notifications.read', description: 'View all notification records (Admin only)' },
];

export const migration: Migration = {
  id: '0010',
  category: 'backfill',
  description: 'Add initial permissions for Notifications module',
  ...
  async apply(db: Db): Promise<void> {
    ...
    // 3. Grant notifications.read to SUPER_ADMIN only
    const adminRole = await rolesCollection.findOne({ name: ROLES.SUPER_ADMIN });
    if (adminRole) {
      const permsToGrant = [permMap.get('notifications.read')].filter(Boolean);
  ...
  async verify(db: Db): Promise<void> {
    ...
    if (count !== PERMISSIONS.length) {
      throw new Error(`Expected ${PERMISSIONS.length} notifications permissions, found ${count}`);
    }
    ...
    const permDocs = await permissionsCollection
      .find({ name: { $in: PERMISSIONS.map((p) => p.name) } })
      .toArray();
    const readPerm = permDocs.find((p) => p.name === 'notifications.read')?._id;
    ...
    if (readPerm) {
      ...
      if (adminRole) {
        ...
        if (!hasPerm) {
          throw new Error('SUPER_ADMIN role missing notifications.read permission');
        }
      }
    }
  },
