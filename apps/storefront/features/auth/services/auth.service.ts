...
import type { LoginFormData, RegisterFormData, ApiResponse } from '@nfi/shared';
...
export const authService = {
  ...
  verifyMfa: async (data: { userId: string, token: string }): Promise<ApiResponse<{ status: 'AUTHENTICATED', accessToken: string }>> => {
  ...
  verifyOtp: async (data: { phone: string, code: string }): Promise<ApiResponse<{ status: 'AUTHENTICATED', accessToken?: string, userId?: string }>> => {
