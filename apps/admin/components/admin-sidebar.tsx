...
export function AdminSidebar() {
  ...
  const renderContent = (isMobile: boolean, isExpanded: boolean) => (
    <div className="flex h-full flex-col overflow-hidden">
      {/* Top Brand Header + Nav — flex-1 so footer never gets pushed off */}
      ...
      {/* User Footer — logout ALWAYS VISIBLE */}
      <div
      ...
      >
        <div
          className={`flex items-center rounded-lg ${isExpanded ? 'gap-2.5 px-1 py-1' : 'flex-col gap-1.5 py-1'}`}
        >
          {/* Avatar — always shown */}
          <div
          ...
          {/* Name, email, role — only when expanded */}
          {isExpanded && (
          ...
          {/* Logout — ALWAYS VISIBLE regardless of expanded state */}
          <button
            type="button"
