...
export class AnalyticsUseCases {
  ...
  async getLeadsFunnel(range: AnalyticsDateRange = {}): Promise<LeadsFunnelData> {
    const key = CACHE_KEYS.analytics.leadsFunnel(range);
    return this.cache.getOrSet(key, CACHE_TTL.ANALYTICS_KPI, async () => {
      logger.warn({ key }, 'Leads funnel cache miss. Computing metrics.');
  ...
  async getDesignFunnel(range: AnalyticsDateRange = {}): Promise<DesignFunnelData> {
    const key = CACHE_KEYS.analytics.designFunnel(range);
    return this.cache.getOrSet(key, CACHE_TTL.ANALYTICS_KPI, async () => {
      logger.warn({ key }, 'Design funnel cache miss. Computing metrics.');
  ...
  async getSalesMetrics(range: AnalyticsDateRange = {}): Promise<SalesMetricsData> {
    const key = CACHE_KEYS.analytics.salesMetrics(range);
    return this.cache.getOrSet(key, CACHE_TTL.ANALYTICS_KPI, async () => {
      logger.warn({ key }, 'Sales metrics cache miss. Computing metrics.');
  ...
  async getExecutiveKPIs(range: AnalyticsDateRange = {}): Promise<ExecutiveDashboardKPIs> {
    const key = CACHE_KEYS.analytics.executiveKpis(range);
    return this.cache.getOrSet(key, CACHE_TTL.ANALYTICS_KPI, async () => {
      if (!this.isDbConnected()) {
        return {
      ...
      try {
        ...
        const averageOrderValue =
          orderCount > 0 && rawMtd > 0 ? Math.round(rawMtd / (mtdAgg[0]?.count || 1)) : 14200000;
  ...
  async getRevenueTrends(range: AnalyticsDateRange = {}): Promise<RevenueTimeseriesPoint[]> {
    const key = CACHE_KEYS.analytics.revenueTrends(range);
    return this.cache.getOrSet(key, CACHE_TTL.ANALYTICS_KPI, async () => {
      ...
      const baseRetail = [
        12500000, 18000000, 9500000, 24000000, 16500000, 31000000, 22000000, 19500000, 28000000,
        34000000, 26000000, 38500000, 31500000, 42000000,
      ];
      const baseDesign = [
        25000000, 35000000, 20000000, 45000000, 30000000, 60000000, 40000000, 35000000, 55000000,
        70000000, 50000000, 80000000, 65000000, 90000000,
      ];
      ...
      for (let i = daysCount - 1; i >= 0; i--) {
        ...
        d.setDate(
          d.getDate() -
            i * (range.timeRange === 'quarter' ? 7 : range.timeRange === 'year' ? 30 : 1),
        );
  ...
  async getCustomerCohorts(): Promise<CustomerCohortMetrics> {
    const key = CACHE_KEYS.analytics.cohorts();
    return this.cache.getOrSet(key, CACHE_TTL.ANALYTICS_KPI, async () => {
      const tiers = [
        {
          tier: 'VIP_PLATINUM',
          label: 'VIP Platinum (₹15L+)',
          count: 18,
          revenueContribution: 1420000000,
          percentage: 42.5,
        },
        {
          tier: 'HIGH_NET_WORTH',
          label: 'High Net Worth (₹5L - ₹15L)',
          count: 34,
          revenueContribution: 1120000000,
          percentage: 33.5,
        },
        {
          tier: 'COMMERCIAL',
          label: 'Commercial Architects',
          count: 12,
          revenueContribution: 560000000,
          percentage: 16.8,
        },
        {
          tier: 'RETAIL',
  ...
  async getCategoryPerformance(): Promise<CategoryPerformanceItem[]> {
    const key = CACHE_KEYS.analytics.categoryPerf();
    return this.cache.getOrSet(key, CACHE_TTL.ANALYTICS_KPI, async () => {
      const items: CategoryPerformanceItem[] = [
        {
          categoryId: 'cat_teak',
          categoryName: 'Solid Teak & Rosewood Furniture',
          totalRevenue: 1650000000,
          unitsSold: 46,
          sharePercentage: 48.5,
          grossMarginPercent: 54,
        },
        {
          categoryId: 'cat_panels',
          categoryName: 'Acoustic Fluted Wall Panels',
          totalRevenue: 880000000,
          unitsSold: 72,
        ...
        },
        {
          ...
          categoryName: 'Handcrafted Brass Accents & Partitions',
        ...
        },
        {
          ...
          categoryName: 'Bespoke Heirlooms & Lighting',
          totalRevenue: 350000000,
          unitsSold: 35,
          sharePercentage: 10.4,
          grossMarginPercent: 49,
        },
      ];
