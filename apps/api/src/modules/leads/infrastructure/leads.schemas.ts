...
import { Lead, LeadStatus, LeadPriority, LeadSource, LeadInterestType, LeadProjectType, LeadTimeline, MarketingChannel } from '../domain/leads.types';
...
const LeadSchema = new Schema<ILeadDocument>(
  {
    ...
    projectType: { 
      type: String, 
      enum: ['RESIDENTIAL', 'COMMERCIAL', 'MODULAR_KITCHEN', 'BEDROOM', 'LIVING_ROOM', 'HOTEL', 'RESTAURANT', 'INSTITUTION'] 
    },
    budgetRange: {
      min: { type: Number },
      max: { type: Number }
    ...
    status: { 
      type: String, 
      required: true, 
      enum: ['NEW', 'QUALIFIED', 'CONTACTED', 'CONSULTATION_SCHEDULED', 'CONVERTED', 'DISQUALIFIED', 'LOST'],
      default: 'NEW'
    },
